...
export function PeaksHeightChart() {
  ...
  return (
    <ChartFrame
      title="Modern elevation of peaks in the story"
      badge={
        <EpistemicBadge
          kind="verified"
          claim="Modern published summit elevations used for comparison."
          note="These bars are a scale comparison, not measurements made by the nineteenth-century survey. The historical sequence is explained in the text and timeline, where claims can be tied to dated sources."
          sources={[
            {
    ...
    >
      <svg
        ...
        role="img"
        aria-label={`Bar chart comparing mountain heights, with ${s.peak} selected at ${s.height.toLocaleString()} meters`}
      />
      <DetailRow head={`${s.peak} · ${s.height.toLocaleString()} m`} body={s.note} />
    </ChartFrame>
  )
}
