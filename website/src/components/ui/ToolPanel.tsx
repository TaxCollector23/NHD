...
export default function ToolPanel({
  ...
  return (
    <section
      id={id}
      className="scroll-mt-28 border-t border-earth-500/20 pt-7 mt-7 first:border-0 first:pt-0 first:mt-0"
    >
      <div className="flex items-baseline gap-3 mb-1">
        <span className="field-num">{String(n).padStart(2, '0')}</span>
        <h2 className="section-title">{title}</h2>
      </div>
      <p className="lede mb-4">{question}</p>

      <div className="grid lg:grid-cols-[minmax(0,1.55fr)_minmax(0,0.95fr)] gap-5 items-start">
        ...
        <aside className="lg:sticky lg:top-24 space-y-3 min-w-0">
          <div className="card-parchment p-4">
            ...
            <div className="mt-3 space-y-3 body-text max-w-none">{explain}</div>
          </div>

          {readouts && readouts.length > 0 && (
            <div className="rounded-md border border-ink-900/15 bg-ink-900 text-parchment-50 p-4">
              <div className="text-[0.75rem] uppercase tracking-[0.2em] text-brass-400 mb-2">Live result</div>
