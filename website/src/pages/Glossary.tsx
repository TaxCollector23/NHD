  ...
  return (
    <div className="container-museum py-8 md:py-10">
      ...
      <p className="mt-5 max-w-2xl text-lg text-ink-800/85 leading-relaxed">
        The technical terms used on this site, with short definitions. Underlined terms in the chapters open the same
        definitions.
      </p>
      ...
      <div className="mt-10 md:columns-2 md:gap-x-8">
        {groups.map(([letter, terms]) => (
          <div key={letter} className="mb-6 break-inside-avoid">
            ...
            <dl className="space-y-4">
              {terms.map((t) => (
                <div key={t.term} id={slug(t.term)} className="scroll-mt-24">
        ...
        {!groups.length && (
          <div className="text-earth-600">No term matches "{q}".</div>
        )}
      </div>
    </div>
  )
