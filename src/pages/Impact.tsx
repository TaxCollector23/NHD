...
const people: Person[] = [
  ...
  },
  {
    key: 'teams',
    func: 'The labour',
    name: 'Indian survey personnel',
    dates: '1802 to 1871',
    role: 'Observers, chain carriers, computers, instrument makers',
    stage: 'Every stage',
    imgSrc: '/portraits/teams.jpg',
        ...
        <div className="mt-8 flex flex-wrap items-center gap-x-2 gap-y-2">
          {['Field observation', 'Instrument reading', 'Computing office', 'Published position'].map((stage, i) => (
            <span key={stage} className="flex items-center gap-2">
              <span className="rounded border border-earth-500/30 bg-parchment-100 px-3 py-1.5 font-mono text-[0.85rem] text-earth-700">
                {stage}
              </span>
          ...
          <ul className="space-y-2 md:col-span-1">
            {people.map((p) => (
              <li key={p.key}>
                ...
                >
                  <div
                    className={`font-display text-[1.2rem] leading-tight ${active === p.key ? 'text-brass-300' : 'text-brass-700'}`}
                  >
                    {p.func}
                  </div>
                  <div
                    className={`text-[0.98rem] leading-tight mt-0.5 ${active === p.key ? 'text-parchment-50' : 'text-ink-900'}`}
                  >
                    {p.name}
                  </div>
                  <div className={`text-[0.85rem] ${active === p.key ? 'text-parchment-200' : 'text-earth-600'}`}>
                    {p.dates}
                  </div>
                </button>
              </li>
            ...
            <div className="grid sm:grid-cols-[150px_1fr] gap-6">
              ...
              <div>
                <div className="page-eyebrow">
                  {person.func} · {person.stage}
                </div>
                <h3 className="font-display text-[2.1rem] mt-1 leading-tight">{person.name}</h3>
                <div className="note-text italic">
                  {person.role} · {person.dates}
                </div>
