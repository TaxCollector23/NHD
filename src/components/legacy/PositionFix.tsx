...
export default function PositionFix({ onReadout }: { onReadout?: (r: Readout[]) => void } = {}) {
  ...
  const bearings = useMemo(() => {
    return HILLS.map((h, i) => {
      const dx = TRUE_POS.x - h.x, dy = TRUE_POS.y - h.y
    ...
    <div className="card-parchment p-6">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
        <div>
          ...
          <div className="text-xs text-earth-600 mt-1">Same problem, two eras, different references. Press <b>Take a fix</b>.</div>
        </div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        >
          <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto bg-parchment-50 rounded">
            ...
            {HILLS.map(h => (
              <g key={h.name}>
                <polygon points={`${h.x - 10},${h.y + 6} ${h.x},${h.y - 8} ${h.x + 10},${h.y + 6}`}
                  fill="rgba(91,58,30,0.25)" stroke="#5b3a1e" strokeWidth="1" />
                <text x={h.x} y={h.y + 22} textAnchor="middle" fontSize="10" fill="#5b3a1e" fontWeight="600">{h.name}</text>
              </g>
            ))}
            ...
            {bearings.map((b, i) => {
              ...
              return (
                <line key={i} x1={b.hill.x} y1={b.hill.y} x2={endX} y2={endY}
                  stroke="#a8802a" strokeWidth="1.3" strokeDasharray="4 3" opacity={0.85} />
              )
            })}
            ...
            {revealed && (
              <>
                ...
                <text x={TRUE_POS.x + 10} y={TRUE_POS.y - 8} fontSize="10" fill="#0f1a2b">true</text>
              </>
            )}
            ...
            {revealed && fix1850 && (
              <>
                ...
                <text x={fix1850.x + 10} y={fix1850.y + 12} fontSize="10" fill="#7a5230" fontWeight="600">fix</text>
              </>
            )}
          </svg>
        </Panel>
          ...
          <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto bg-parchment-50 rounded">
            ...
            {SATS.map(s => (
              <g key={s.name}>
                ...
                <line x1={s.x + 8} y1={s.y - 8} x2={s.x - 8} y2={s.y + 8} stroke="#243449" strokeWidth="2" />
                <text x={s.x} y={s.y - 12} textAnchor="middle" fontSize="9" fill="#243449" fontWeight="600">{s.name}</text>
              </g>
            ))}
            ...
            {ranges.map((r, i) => (
              <circle key={i}
                cx={r.sat.x} cy={r.sat.y}
                r={r.r}
                fill="none" stroke="#4b7a8a" strokeWidth="1" strokeDasharray="3 3" opacity={0.7} />
            ))}
            ...
            {revealed && fixGPS && (
              <>
                ...
                <text x={fixGPS.x + 10} y={fixGPS.y + 12} fontSize="10" fill="#7a5230" fontWeight="600">fix</text>
              </>
            )}
          </svg>
        </Panel>
      ...
      <div className="grid md:grid-cols-2 gap-4 mt-4">
        <div className="rounded-md border border-earth-500/20 bg-parchment-50 p-3 space-y-3 text-sm">
          <Field label="Bearing measurement error (°)" v={angleErr} setV={setAngleErr} min={0} max={2} step={0.05} decimals={2} />
          <p className="text-xs text-earth-600">
            A hand compass is coarse. A tripod-mounted instrument read carefully and averaged over
            repeated sightings is far finer. This site prints no figure for the survey's own instruments,
            because none could be traced to a document.
          </p>
          {revealed && fix1850 && (
            <div className="text-xs text-earth-600 leading-snug">
              The three bearings rarely cross at a single point. The small triangle between them
              is the uncertainty. Wider bearing error → larger triangle.
            </div>
          )}
        </div>

        <div className="rounded-md border border-earth-500/20 bg-parchment-50 p-3 space-y-3 text-sm">
          <Field label="Range measurement error (m)" v={rangeErr} setV={setRangeErr} min={0.5} max={15} step={0.5} decimals={1} />
          <p className="text-xs text-earth-600">
            Move the slider to see how noise in a distance measurement moves the fix. The units here are
            canvas units, not metres: this is geometry, not a benchmark.
          </p>
          {revealed && fixGPS && (
            <div className="text-xs text-earth-600 leading-snug">
              The range circles overlap in a small region, and its size is the uncertainty. Same idea as
              the bearings, but the doubt is ring-shaped, not wedge-shaped.
            </div>
          )}
        </div>
      </div>
      ...
      <p className="text-xs text-earth-600 mt-4 leading-relaxed">
        Same idea, different references: hilltops and a compass then, satellites and a timed signal now.
        The technology changed completely; the geometry of fixing yourself by intersection did not.
      </p>
    </div>
  )
}
...
function Panel({
  era, badge, methodNote, children,
}: {
  era: string; badge: string; methodNote: string;
...
function Field({ label, v, setV, min, max, step, decimals }:
...
function lineLineIntersection(a: { hill: { x: number, y: number }, angle: number },
                              b: { hill: { x: number, y: number }, angle: number }) {
  const dx1 = Math.cos(a.angle), dy1 = Math.sin(a.angle)
  const dx2 = Math.cos(b.angle), dy2 = Math.sin(b.angle)
...
function circleCircle(a: { sat: { x: number, y: number }, r: number },
                      b: { sat: { x: number, y: number }, r: number }) {
  const dx = b.sat.x - a.sat.x, dy = b.sat.y - a.sat.y
