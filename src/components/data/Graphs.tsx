...
const PEAKS: Peak[] = [
  { peak: 'Chimborazo (Andes)',    short: 'Chimborazo',    height: 6263, thought: 1800,
    note: 'Long thought the highest. Its bulge is the farthest point on the surface from the centre of the Earth.' },
  { peak: 'Dhaulagiri (Nepal)',    short: 'Dhaulagiri',    height: 8167, thought: 1808,
    note: 'Briefly held the title after a measurement in 1808.' },
  { peak: 'Kanchenjunga (Sikkim)', short: 'Kanchenjunga',  height: 8586, thought: 1849,
    note: 'Held the title through the 1840s, until Peak XV.' },
  { peak: 'Peak XV → Everest',     short: 'Peak XV',       height: 8848, thought: 1856,
...
export function PeaksHeightChart() {
  ...
  const s = PEAKS[sel]
  return (
    <ChartFrame title="Which peak was believed tallest, and when"
                eyebrow="A moving target"
                badge={<EpistemicBadge kind="verified"
                  ...
                  sources={[{ text: 'Modern published elevations (e.g. Survey of India / standard references)', type: 'Secondary' }, { text: 'Keay, The Great Arc; Edney, Mapping an Empire', type: 'Secondary' }]} />}
      ...
      <DetailRow
        head={`${s.peak} · ${s.height.toLocaleString()} m · believed tallest c. ${s.thought}`}
        body={s.note}
      />
    </ChartFrame>
  )
...
function ChartFrame({
  title, eyebrow, caption, badge, children,
}: { title: string; eyebrow: string; caption: string; badge?: React.ReactNode; children: React.ReactNode }) {
  return (
...
function renderPeaks(node: SVGSVGElement, opts: {
  width: number; height: number;
  data: Peak[];
  highlightIdx?: number; onSelect?: (i: number) => void;
}) {
  ...
  const x = d3.scaleBand<string>().domain(data.map(d => d.short)).range([0, w]).padding(0.35)
  const y = d3.scaleLinear().domain([0, 9500]).range([h, 0])

  g.append('g').attr('transform', `translate(0,${h})`).call(d3.axisBottom(x)).attr('color', EARTH)
    .selectAll('text').style('font-size', 10)
  g.append('g').call(d3.axisLeft(y).ticks(6).tickFormat(d => `${d} m`)).attr('color', EARTH)
  g.append('g').selectAll('line').data(y.ticks(6)).join('line')
    .attr('x1', 0).attr('x2', w).attr('y1', d => y(d)).attr('y2', d => y(d))
    .attr('stroke', GRID).attr('stroke-opacity', 0.15).attr('stroke-dasharray', '2 3')
  ...
  data.forEach((d, i) => {
    ...
    const group = g.append('g').style('cursor', 'pointer')
      ...
      .on('mouseleave', function () {
        d3.select(this).select('polygon.mountain').attr('fill-opacity', isActive ? 0.9 : 0.7)
      })

    group.append('polygon').attr('class', 'mountain')
      .attr('points', `${bx},${h} ${bx + bw / 2},${by - 8} ${bx + bw},${h}`)
      .attr('fill', fill).attr('fill-opacity', isActive ? 0.9 : 0.7)
      .attr('stroke', EARTH).attr('stroke-width', isActive ? 1.5 : 1)

    group.append('polygon')
      .attr('points', `${bx + bw / 2 - 12},${by + 12} ${bx + bw / 2},${by - 8} ${bx + bw / 2 + 12},${by + 12}`)
      .attr('fill', PAPER).attr('opacity', 0.85)

    group.append('text').attr('x', bx + bw / 2).attr('y', by - 14).attr('text-anchor', 'middle')
      .attr('font-size', 10).attr('fill', isActive ? EARTH : EARTH).attr('font-weight', 700)
      .text(`${d.height.toLocaleString()} m`)

    group.append('text').attr('x', bx + bw / 2).attr('y', h + 30).attr('text-anchor', 'middle')
      .attr('font-size', 9).attr('fill', EARTH).attr('font-style', 'italic')
