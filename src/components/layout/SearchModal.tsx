...
import { Search, X, Command, ArrowRight, BookText, Compass, MapIcon, Mountain, Wrench, ScrollText, Library } from 'lucide-react'
...
export default function SearchModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-ink-900/50 backdrop-blur-sm p-4 flex items-start justify-center animate-page-in"
         onClick={onClose}>
      <div className="w-full max-w-xl mt-[10vh] card-parchment overflow-hidden shadow-2xl" onClick={e => e.stopPropagation()}>
        <div className="flex items-center gap-3 border-b border-earth-500/25 px-4 py-3">
          ...
          <input
            ...
            aria-label="Search"
          />
          <kbd className="hidden sm:inline text-[10px] text-earth-600 border border-earth-500/30 rounded px-1.5 py-0.5">ESC</kbd>
        ...
        <ul className="max-h-[60vh] overflow-y-auto py-2">
          ...
          {!results.length && (
            <li className="px-4 py-6 text-center text-sm text-earth-600">
              Nothing matches “{q}”.
            </li>
          )}
        </ul>
