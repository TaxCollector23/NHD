...
const STATIONS: Station[] = [
  { id: 'long',  name: 'Long low-angle sight-line', distance: 174, angle: 2.18, observer: 0.137, blurb: 'A distant lowland station: an apparent angle of just over 2° across ~170 km still resolves a ~9 km peak. Illustrative values, not a historical record.' },
  { id: 'long2', name: 'Neighbouring low station',  distance: 176, angle: 2.13, observer: 0.137, blurb: 'Averaging several such readings from different stations cancels random error. Illustrative values.' },
  { id: 'ridge', name: 'High ridge station',        distance: 130, angle: 3.10, observer: 2.13,  blurb: 'A higher observer shortens the curvature correction. Illustrative values.' },
  { id: 'demo',  name: 'Nearby hill',               distance: 40,  angle: 5.0,  observer: 0.6,   blurb: 'Short baseline, large angle: the corrections are small. Illustrative values.' },
]
...
export default function EverestCalculator({ onReadout }: { onReadout?: (r: Readout[]) => void } = {}) {
  ...
  const applyStation = (s: Station) => {
    setStationId(s.id); setDistance(s.distance); setAngle(s.angle); setObserver(s.observer)
  }
  ...
  const canvasW = 640, canvasH = 320
  const padL = 60, padR = 56, padT = 26, padB = 66
    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="mb-2 flex items-center gap-3 flex-wrap">
          <span className="page-eyebrow flex items-center gap-2"><BookOpen className="w-3 h-3" /> Worked examples</span>
          <EpistemicBadge kind="illustrative"
            ...
            sources={[{ text: 'Trig heighting + curvature/refraction (Bomford, Geodesy)', type: 'Secondary' }, { text: 'The real Peak XV observation table, in Waugh 1851', type: 'Primary', needed: true }]} />
        </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="card-parchment p-4">
          <svg viewBox={`0 0 ${canvasW} ${canvasH}`} className="w-full h-auto">
            {/* sea level */}
            <line x1={obsX - 12} y1={groundY} x2={peakX + 40} y2={groundY} stroke="#7a5230" strokeWidth="1" opacity="0.5" />
            <text x={obsX - 12} y={groundY + 14} fontSize="9" fill="#7a5230">sea level</text>

            {/* observer eye-level horizontal (the naive reference) */}
            <line x1={obsX} y1={obsY} x2={peakX} y2={obsY} stroke="#c2a06a" strokeWidth="1" strokeDasharray="3 3" opacity="0.7" />
            {/* curved horizon — Earth bending away, driven by distance & k */}
            <path d={`M ${obsX} ${obsY} Q ${(obsX + peakX) / 2} ${obsY + curvPx * 0.6} ${peakX} ${obsY + curvPx}`}
              fill="none" stroke="#7a5230" strokeDasharray="2 4" opacity="0.7" />
            ...
            {obsY < groundY - 2 && (
              <>
                ...
                <text x={obsX - 6} y={(groundY + obsY) / 2} fontSize="10" fill="#243449" textAnchor="end">h₀</text>
              </>
            )}

            {/* mountain — apex at the CORRECTED (true) peak, base at sea level */}
            <polygon points={`${peakX - 52},${groundY} ${peakX},${correctedPeakY} ${peakX + 52},${groundY}`}
              fill="rgba(91,58,30,0.14)" stroke="#5b3a1e" strokeWidth="1.5" />
            <polygon points={`${peakX - 20},${correctedPeakY + (groundY - correctedPeakY) * 0.26} ${peakX},${correctedPeakY} ${peakX + 20},${correctedPeakY + (groundY - correctedPeakY) * 0.26}`}
              fill="#f5eddc" opacity="0.85" />
            ...
            <path d={`M ${obsX + arcR} ${obsY} A ${arcR} ${arcR} 0 0 0 ${arcEndX} ${arcEndY}`}
              fill="none" stroke="#a8802a" strokeWidth="1.4" />
            <text x={obsX + arcR + 6} y={obsY - 6} fontSize="11" fill="#7a5230">θ = {angle.toFixed(2)}°</text>
            ...
            {curvPx > 3 && (
              <>
                ...
                <text x={peakX + 8} y={(peakY + correctedPeakY) / 2 + 3} fontSize="9" fill="#7a2020">+curve &amp; refraction</text>
              </>
            )}

            {/* distance label */}
            <text x={(obsX + peakX) / 2} y={groundY + 26} textAnchor="middle" fontSize="12" fill="#243449" fontWeight="600">
        ...
        <div className="space-y-4">
          <Field label="Distance to peak (km)"     v={distance} setV={setDistance} min={10}  max={250} step={1}    decimals={0}
            help="Chained-triangle distance from your observation station to the peak." />
          <Field label="Apparent elevation angle (°)" v={angle} setV={setAngle}    min={0.05} max={10}  step={0.005} decimals={3}
            help="What the theodolite reads. Tiny angles carry huge heights across long baselines." />
          <Field label="Observer altitude (km)"    v={observer} setV={setObserver} min={0}    max={5}   step={0.01}  decimals={2}
            help="Your altitude above sea level. Peak height is measured from sea level, not from your feet." />
          <Field label="Refraction coefficient k"   v={refraction} setV={setRefraction} min={0.10} max={0.20} step={0.01} decimals={2}
          ...
          <div className="rounded-md border border-earth-500/20 bg-parchment-50 p-4 text-sm space-y-1.5">
            <div className="flex justify-between">
              <span><span className="field-num mr-1">①</span> Basic geometry <span className="text-earth-600 text-xs">(flat, airless Earth)</span></span>
              <b>{heightMeters.toFixed(0)} m · {(heightMeters * 3.28084).toFixed(0)} ft</b>
            </div>
            <div className="flex justify-between">
              <span><span className="field-num mr-1">②</span> + Earth's curve & refraction <span className="text-earth-600 text-xs">(+{curvatureDrop.toFixed(0)} m)</span></span>
              <b className="text-brass-600">{correctedM.toFixed(0)} m · {(correctedM * 3.28084).toFixed(0)} ft</b>
            </div>
      ...
      <div className="card-parchment p-5">
        <div className="flex items-center gap-2 mb-2"><Info className="w-4 h-4 text-brass-600" /><div className="page-eyebrow">Why the correction matters</div></div>
        <p className="text-sm text-ink-800/90 leading-relaxed">
          Over long distances the Earth curves away, hiding the peak's base below the observer's
          straight-line horizon; atmospheric refraction bends light the other way and cancels part of
          it. Both fold into one coefficient <em>k</em> ≈ 0.13 (Bomford, <i>Geodesy</i>), and getting
          it wrong throws the height by hundreds of metres. Move the <em>k</em> slider to see it.
        </p>
      </div>
    </div>
  )
...
function Field({ label, v, setV, min, max, step, decimals, help }:
  { label: string, v: number, setV: (n: number) => void, min: number, max: number, step: number, decimals: number, help?: string }) {
  return (
    <div>
