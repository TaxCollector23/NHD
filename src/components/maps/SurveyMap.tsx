...
import { GREAT_ARC_PX } from '../../lib/projection'
import EpistemicBadge from '../ui/EpistemicBadge'
import UpgradeNote from '../ui/UpgradeNote'
...
export default function SurveyMap() {
  ...
  const latestStation = useMemo(
    () => [...visibleStations].sort((a, b) => b.year - a.year)[0],
    [visibleStations]
  )
  ...
  const restart = () => { setYear(YEAR_MIN); setPlaying(true) }
    ...
    <div className="card-parchment p-5 md:p-6">
      <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
        ...
        <EpistemicBadge kind="illustrative"
          claim="A schematic of a triangulation network growing over seven decades."
          note="Station coordinates are real places. The triangle mesh and the year each link appears are a simplified teaching diagram, not the survey's actual network, which had hundreds of principal triangles in a different configuration."
          sources={[
            { text: 'Station latitude and longitude (standard geographic references)', type: 'Secondary' },
            { text: 'Actual triangulation chart, Historical Records of the Survey of India', type: 'Primary', needed: true },
          ]} />
      </div>
      ...
      <div className="grid lg:grid-cols-[minmax(0,1.55fr)_minmax(0,1fr)] gap-6">
        <div className="min-w-0">
          <div className="relative rounded-md border border-earth-500/25 overflow-hidden bg-parchment-50">
            <svg viewBox={INDIA_VIEWBOX} className="w-full h-auto block" role="img" aria-label="Survey stations appearing over time">
              <IndiaBackground showArc />
              <text x={(GREAT_ARC_PX[1]?.x ?? 300) + 14} y={((GREAT_ARC_PX[0]?.y + GREAT_ARC_PX[1]?.y) / 2) || 380}
                    fontSize="13" fill="#5b3a1e" opacity="0.8" fontFamily="Georgia, serif">The Great Arc</text>
              ...
              {visibleTriangles.map(([a, b, c], i) => {
                ...
                return (
                  <motion.polygon
                    key={`${a}-${b}-${c}`}
                    initial={{ opacity: 0 }} animate={{ opacity: 0.9 }} transition={{ duration: 0.5, delay: i * 0.02 }}
                    points={`${sa.x},${sa.y} ${sb.x},${sb.y} ${sc.x},${sc.y}`}
                    fill="rgba(200,155,58,0.10)" stroke="#a8802a" strokeWidth="1.3" strokeLinejoin="round"
                  />
                )
              })}
              ...
              {visibleStations.map(s => {
                ...
                return (
                  ...
                  >
                    <circle cx={s.x} cy={s.y} r={isFocus ? 7.5 : (isSelected ? 6 : 4.5)}
                      fill={isFocus || isSelected ? '#c89b3a' : '#0f1a2b'}
                      stroke={isFocus || isSelected ? '#5b3a1e' : '#f6efe0'}
                      strokeWidth="1.4" />
                    {(isFocus || isSelected) && (
                      <g>
                        <rect x={s.x + dx - (ta === 'end' ? 138 : 0)} y={s.y + dy - 15} width="138" height="23" rx="4"
                          fill="#0f1a2b" opacity="0.92" />
                        <text x={s.x + dx + (ta === 'end' ? -8 : 8)} y={s.y + dy + 1}
                          textAnchor={ta} fontSize="13" fill="#f5eddc" fontWeight="600">
                          {s.name}
                        </text>
                      </g>
                    )}
        ...
        <div className="space-y-5">
          <div>
            <label htmlFor="survey-year" className="page-eyebrow block mb-2">Drag to any year</label>
            <input
              id="survey-year"
              type="range" min={YEAR_MIN} max={YEAR_MAX} value={year}
              ...
              {[1802, 1820, 1840, 1852, 1871].map(y => (
                <button key={y} onClick={() => { setPlaying(false); setYear(y) }}
                  className={`px-3 py-1.5 rounded-md border text-[0.9rem] transition-colors ${
                    year === y ? 'bg-ink-900 text-parchment-50 border-ink-900'
                               : 'border-earth-500/30 hover:bg-parchment-200'
                  }`}>
                  {y}
                </button>
              ))}
              <button onClick={() => { setYear(YEAR_MIN); setPlaying(false) }}
          ...
          <div className="rounded-md border border-earth-500/20 bg-parchment-50 p-4 min-h-[8rem]">
            <div className="page-eyebrow mb-1.5">
              {hovered ? 'Hovered' : (selected && stations.find(s => s.id === selected && s.year <= year) ? 'Selected station' : 'Most recent station')}
            </div>
            {focused ? (
              <>
          ...
          <div className="rounded-md border border-earth-500/20 bg-parchment-50 p-4 space-y-2 caption-text">
            <div className="flex justify-between"><span>Stations shown</span><b className="font-mono">{visibleStations.length}</b></div>
            <div className="flex justify-between"><span>Triangles drawn</span><b className="font-mono">{visibleTriangles.length}</b></div>
          </div>
        </div>
      </div>

      <UpgradeNote what="A real version of this map needs the station list and the observed triangles of the principal triangulation: names, coordinates, and the year each was fixed." />
    </div>
  )
}
