...
- [ ] NHDWebCentral shows a **live word count** on the site automatically. Confirm it is **≤ 1,200**. The React source is currently at **1,168** (`npm run words` — re-check this yourself before relying on it, since the number moves as the prose is edited); if the NHDWebCentral count differs, trim captions first.
